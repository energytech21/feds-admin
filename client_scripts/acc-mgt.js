      ...
      { data: "status" },
      {
        ...
        render: (data, type, row, meta) => {
          ...
          if (state == "Active") {
            return (data =
              `<div class='ui three buttons'>
                            <a class='ui animated fade negative ${(row.users_id == $("#accounts").data('value') ? "disabled" : "")} button'  onClick='update_status(${data},2)'>
                                <div class='visible content'><i class='times circle outline icon'></i></div>
...
function editUser(users_id) {
  $(document).ready(function() {
    $.post("/search",{uid : users_id},(response,status,jqXHR)=>{

      $('#regform').form('set values',{
        fname:response[0].user_fname,
        lname:response[0].user_lname,
        mname:response[0].user_mname,
        address:response[0].address,
        uname:response[0].user_username,
        pword:response[0].user_password
    });
...
function submitNewForm(){
  var values = $("#regNewform").serialize();
  $.ajax({
      type: "post",
      data: values,
      url: '../register',
